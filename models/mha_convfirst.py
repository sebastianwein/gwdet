...
class MHAModel(LightningModule):
    def __init__(self, 
        ...
        self.pos_enc = PosEncoding(dim0=self.hparams.num_tokens, 
                                   dim1=self.hparams.embed_dim, 
                                   mode=self.hparams.pos_enc)
    ...
    def forward(self, 
        ...
        batches, dets, _ = x.shape
        x = x.flatten(0, 1).unsqueeze(-2)      # (b*dets, 1, l)
        x = self.conv(x)                       # (b*dets, c, l')
        x = x.unflatten(0, (batches, dets))    # (b, dets, c, l')
        x = self.tokenize(x)                   # (b, dets, c, t, token_length)
        x = x.transpose(2, 3).transpose(1, 2)  # (b, t, dets, c, token_length)
        x = x.flatten(2)                       # (b, t, dets*c*token_length)
        x = self.embed(x)                      # (b, tokens, embed_dim)    
